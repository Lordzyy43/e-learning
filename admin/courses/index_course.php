...
<body class="bg-gray-100 font-inter min-h-screen flex">
  ...
  <main class="flex-1 ml-64 p-10">
    <div class="flex justify-between items-center mb-6">
      ...
      <a href="add_course.php" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg shadow">+ Tambah Kursus</a>
    </div>
          ...
          while ($row = mysqli_fetch_assoc($result)) {
              echo "<tr class='border-b hover:bg-gray-50 transition'>
                      ...
                      <td class='py-2 px-4 text-center space-x-2'>
                        <a href='edit_course.php?id={$row['id']}' class='bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded-md'>Edit</a>
                        <a href='delete_course.php?id={$row['id']}' onclick=\"return confirm('Yakin ingin menghapus kursus ini?');\" class='bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded-md'>Hapus</a>
                      </td>
                    </tr>";
              $no++;
          }
